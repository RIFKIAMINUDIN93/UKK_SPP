...
Public Class KelolaTransaksi
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        connect()
        If txtnama.Text = "" Or txtidpet.Text = "" Or txtnisn.Text = "" Or txtbln.Text = "" Or txtthn.Text = "" Or cmbidspp.Text = "" Or txtjml.Text = "" Then
            MsgBox("Pastikan mengisi semua field")
        Else
            sql = "Insert into pembayaran(id_pembayaran, id_petugas, nisn, tgl_bayar, bulan_dibayar, tahun_dibayar, id_spp, jumlah_bayar) VALUES ('" & txtnama.Text & "', '" & txtidpet.Text & "', '" & txtnisn.Text & "', '" & Format(Dttglbayar.Value, "yyyy-MM-dd") & "', '" & txtbln.Text & "' , '" & txtthn.Text & "', '" & cmbidspp.Text & "' , '" & txtjml.Text & "')"
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        If txtnama.Text = "" Or txtidpet.Text = "" Or txtnisn.Text = "" Or txtbln.Text = "" Or txtthn.Text = "" Or cmbidspp.Text = "" Or txtjml.Text = "" Then
            MsgBox("Pastikan mengisi semua field")
        Else
            sql = "UPDATE pembayaran set id_petugas='" & txtidpet.Text & "', nisn='" & txtnisn.Text & "', tgl_bayar='" & Format(Dttglbayar.Value, "yyyy-MM-dd") & "', bulan_dibayar='" & txtbln.Text & "' , tahun_dibayar='" & txtthn.Text & "', id_spp='" & cmbidspp.Text & "' , jumlah_bayar='" & cmbidspp.Text & "' WHERE id_pembayaran='" & txtnama.Text & "'"
            cmd = New SqlCommand(sql, con)
            cmd.ExecuteNonQuery()
    ...
    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        ...
        If txtnama.Text = "" Or txtidpet.Text = "" Or txtnisn.Text = "" Or txtbln.Text = "" Or txtthn.Text = "" Or cmbidspp.Text = "" Or txtjml.Text = "" Then
            MsgBox("Pastikan mengisi semua field")
        Else
            sql = " DELETE FROM pembayaran WHERE id_pembayaran='" & txtnama.Text & "'"
            cmd = New SqlCommand(sql, con)
            cmd.ExecuteNonQuery()
    ...
    Private Sub dgv_user_MouseClick(sender As Object, e As MouseEventArgs) Handles dgv_user.MouseClick
        txtnama.Text = dgv_user.Rows(dgv_user.CurrentRow.Index).Cells(0).Value
    ...
    Private Sub KelolaTransaksi_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        connect()
        aturdgv()
    End Sub
    Private Sub txtcari_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtcari.KeyPress
        sql = "SELECT * From pembayaran Where (nisn like '%" & txtcari.Text & "%')"
        da = New SqlDataAdapter(sql, con)
        ds = New DataSet
        da.Fill(ds, "kelas")
    ...
    Private Sub PD_PrintPage(sender As Object, e As PrintPageEventArgs) Handles PD.PrintPage
        ...
        e.Graphics.DrawString("nisn", f10, Brushes.Black, 0, 95)
        ...
        For baris As Integer = 0 To dgv_user.RowCount - 1
            tinggi += 15
            e.Graphics.DrawString(dgv_user.Rows(baris).Cells(2).Value.ToString, f10, Brushes.Black, 0, 100 + tinggi)
            e.Graphics.DrawString(dgv_user.Rows(baris).Cells(7).Value.ToString, f10, Brushes.Black, 60, 100 + tinggi)
            e.Graphics.DrawString(dgv_user.Rows(baris).Cells(3).Value.ToString, f10, Brushes.Black, 200, 100 + tinggi)


        Next
        tinggi = 110 + tinggi
        e.Graphics.DrawString(garis, f10, Brushes.Black, 0, 400)
        e.Graphics.DrawString("Hatur Thank You !!!!!", f10, Brushes.Black, centermargin, 450, tengah)
    End Sub
    ...
    Private Sub txtidpem_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtnama.KeyPress
